...
export class TypesUsageEvaluator {
  ...
  private readonly usageResultCache: Map<ts.Symbol, Map<ts.Symbol, boolean>> =
    new Map();
  ...
  private computeUsageForNode(node: ts.Node): void {
    ...
    if (isNodeNamedDeclaration(node)) {
      ...
      if (nodeName !== undefined) {
        if (
          ts.isObjectBindingPattern(nodeName) ||
          ts.isArrayBindingPattern(nodeName)
        ) {
      ...
      for (const exportElement of node.exportClause.elements) {
        ...
        if (namespaceImportForElement !== undefined) {
          ...
          this.addUsagesForNamespacedModule(
            ...
            namespaceImportForElement.parent.parent
  ...
  private computeUsagesRecursively(
    ...
    ts.forEachChild(parent, (child: ts.Node) => {
      ...
      if (
        ...
        if (
          ts.isBindingElement(child.parent) &&
          child.parent.propertyName === child
        ) {
          return;
        }
