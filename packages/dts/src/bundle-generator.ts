...
import { normalLog, verboseLog, warnLog } from "./logger";
...
export function generateDtsBundle(
  ...
  const { program, rootFilesRemapping } = compileDts(
    entries.map((entry: EntryPointConfig) => entry.filePath),
  ...
  return entries.map((entryConfig: EntryPointConfig) => {
    ...
    const rootSourceFileSymbol =
      typeChecker.getSymbolAtLocation(rootSourceFile);
    ...
    const rootFileExportSymbols = rootFileExports.map(
      (exp: SourceFileExport) => exp.symbol,
    );
    ...
    function updateResultForAnyModule(
      ...
      function updateResultForExternalExport(
        ...
        for (const declaration of getDeclarationsForExportedValues(
          ...
          if (
            ts.isExportDeclaration(exportAssignment) &&
      ...
      function updateResultImpl(
        ...
        for (const statement of statementsToProcess) {
          ...
          if (
            ...
            currentModule.type === ModuleType.ShouldBeInlined
          ) {
          ...
          if (ts.isExportDeclaration(statement)) {
            ...
            if (
              ...
              currentModule.type === ModuleType.ShouldBeImported
            ) {
          ...
          switch (currentModule.type) {
            case ModuleType.ShouldBeReferencedAsTypes:
              ...
              forEachNodeThatShouldBeImported(statement, () =>
            ...
            case ModuleType.ShouldBeInlined:
              if (ts.isVariableStatement(statement)) {
                ...
                if (statementName !== undefined) {
                  collisionsResolver.addTopLevelIdentifier(
                    statementName as ts.Identifier | ts.DefaultKeyword,
                  );
                }
              }
    ...
    function isReferencedModuleImportable(
      statement: ts.ExportDeclaration | ts.ImportDeclaration,
    ): boolean {
      return (
        getReferencedModuleInfo(statement, criteria, typeChecker)?.type ===
        ModuleType.ShouldBeImported
      );
    }

    function handleExportDeclarationFromRootModule(
      exportDeclaration: ts.ExportDeclaration,
    ): void {
      function handleExportStarStatement(
        exportStarStatement: ts.ExportDeclaration,
        visitedSymbols: Set<ts.Symbol> = new Set(),
      ): void {
        if (
          exportStarStatement.moduleSpecifier === undefined ||
          exportStarStatement.exportClause !== undefined
        ) {
          throw new Error(
        ...
        switch (referencedModuleInfo.type) {
          case ModuleType.ShouldBeInlined: {
            ...
            const referencedSourceFileExportStarSymbol =
              referencedModuleSymbol.exports?.get(
       ...
       */
      function findExportingExportStarExportFromImportableModule(
        referencedModuleSymbol: ts.Symbol,
        nodeSymbol: ts.Symbol,
      ): ExportingExportStarExport | null {
        function findResultRecursively(
          referencedModuleSym: ts.Symbol,
          exportedNodeSym: ts.Symbol,
          ...
          for (const exportStarDeclaration of getDeclarationsForSymbol(
            ...
            if (isReferencedModuleImportable(exportStarDeclaration)) {
              ...
              const exportedNodeSymbol = referencedModuleExports.find(
                (exp: ts.Symbol) =>
                  getActualSymbol(exp, typeChecker) === nodeSymbol,
              );
      ...
      if (
        ...
        if (exportDeclaration.moduleSpecifier === undefined) {
          for (const exportElement of exportDeclaration.exportClause.elements) {
            ...
            const namespaceImportFromImportableModule =
              getDeclarationsForSymbol(exportElementSymbol).find(
                (
                  importDecl: ts.Declaration,
                ): importDecl is ts.NamespaceImport => {
                  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
                  return (
                    ts.isNamespaceImport(importDecl) &&
                    isReferencedModuleImportable(
                      importDecl.parent.parent as ts.ImportDeclaration,
                    )
                  );
                },
              );
            ...
            if (namespaceImportFromImportableModule !== undefined) {
              ...
              const importModuleSpecifier = getImportModuleName(
                namespaceImportFromImportableModule.parent
        ...
        if (exportDeclaration.moduleSpecifier !== undefined) {
          ...
          for (const exportElement of exportDeclaration.exportClause.elements) {
            ...
            const exportingExportStarResult =
    ...
    function updateResultForRootModule(
      statements: readonly ts.Statement[],
      currentModule: ModuleInfo,
    ): void {
    ...
    function forEachNodeThatShouldBeImported(
      statement: ts.Statement | ts.SourceFile | ts.ExportSpecifier,
      callback: (st: ts.DeclarationStatement) => void,
    ): void {
      ...
      for (const statementToImport of statementsToImport) {
        if (
          shouldNodeBeImported(statementToImport as ts.DeclarationStatement)
        ) {
    ...
    function updateImportsForStatement(
      ...
      forEachNodeThatShouldBeImported(
        statement,
        (statementToImport: ts.DeclarationStatement) => {
    ...
    function getDeclarationUsagesSourceFiles(
      ...
      return new Set(
        getExportedSymbolsUsingStatement(declaration)
          ...
          .reduce(
            (acc: ts.Declaration[], val: ts.Declaration[]) => acc.concat(val),
            [],
          )
    ...
    function addImport(
      ...
      forEachImportOfStatement(
        statement,
        (
          imp: ImportOfStatement,
          referencedModuleInfo: ModuleInfo,
          ...
          if (ts.isExportSpecifier(imp)) {
            ...
            addNamedImport(
              importItem,
              imp.name as ts.Identifier,
              (imp.propertyName || imp.name) as ts.Identifier,
            );
            return;
          }
          ...
          if (ts.isNamespaceExport(imp)) {
            ...
            addNsImport(importItem, imp.name as ts.Identifier);
            return;
          }
    ...
    function forEachImportOfStatement(
      statement: ts.DeclarationStatement | ts.SourceFile,
      callback: (
        imp: ImportOfStatement,
        referencedModuleInfo: ModuleInfo,
      ...
      getDeclarationUsagesSourceFiles(statement).forEach(
        (sourceFile: ts.SourceFile | ts.ModuleDeclaration) => {
          if (
            getModuleLikeModuleInfo(sourceFile, criteria, typeChecker).type !==
            ModuleType.ShouldBeInlined
          ) {
            // we should ignore source files that aren't inlined
            return;
          }

          const sourceFileStatements: readonly ts.Statement[] = ts.isSourceFile(
            sourceFile,
          )
          ...
          sourceFileStatements.forEach((st: ts.Statement) => {
            ...
            if (ts.isExportDeclaration(st) && st.exportClause !== undefined) {
              ...
              if (
                ...
                areDeclarationSame(statement, st.importClause)
              ) {
                // import name from 'module';
                callback(
                  st.importClause,
    ...
    function getInlinedSymbolsUsingSymbol(
      symbol: ts.Symbol,
      predicate: (usedInSymbol: ts.Symbol) => boolean,
    ): ts.Symbol[] {
      return Array.from(
        typesUsageEvaluator.getSymbolsUsingSymbol(symbol) ?? [],
      ).filter((usedInSymbol: ts.Symbol) => {
    ...
    function isSymbolUsedByInlinedSymbols(
      symbol: ts.Symbol,
      predicate: (usedInSymbol: ts.Symbol) => boolean,
      visitedSymbols: Set<ts.Symbol> = new Set(),
    ): boolean {
    ...
    function isSymbolUsedByRootFileExports(symbol: ts.Symbol): boolean {
      return rootFileExportSymbols.some((rootSymbol: ts.Symbol) =>
        typesUsageEvaluator.isSymbolUsedBySymbol(symbol, rootSymbol),
      );
    }
    ...
    function isNodeUsed(node: ts.Node): boolean {
      if (isNodeNamedDeclaration(node) || ts.isSourceFile(node)) {
        ...
        const nodeUsedByDirectExports =
          isSymbolUsedByRootFileExports(nodeSymbol);
        ...
        if (
          inlineDeclareGlobals &&
          isSymbolUsedByInlinedSymbols(nodeSymbol, isSymbolForGlobalDeclaration)
        ) {
          return true;
        }
    ...
    function shouldSymbolBeImported(nodeSymbol: ts.Symbol): boolean {
      const isSymbolDeclaredInDefaultLibrary = getDeclarationsForSymbol(
        nodeSymbol,
      ).some((declaration: ts.Declaration) =>
    ...
    function getExportedSymbolsUsingSymbol(
      nodeSymbol: ts.Symbol,
    ): readonly ts.Symbol[] {
      const symbolsUsingNode =
        typesUsageEvaluator.getSymbolsUsingSymbol(nodeSymbol);
    ...
    function areDeclarationSame(
      left: ts.NamedDeclaration,
      right: ts.NamedDeclaration,
    ): boolean {
      const leftSymbols = splitTransientSymbol(
        getNodeSymbol(left, typeChecker) as ts.Symbol,
        typeChecker,
      );
      const rightSymbols = splitTransientSymbol(
        getNodeSymbol(right, typeChecker) as ts.Symbol,
        typeChecker,
      );
    ...
    function createNamespaceForExports(
      exports: ts.SymbolTable,
      namespaceSymbol: ts.Symbol,
    ): string | null {
      function addSymbolToNamespaceExports(
        ...
        if (
          !typesUsageEvaluator.isSymbolUsedBySymbol(symbol, namespaceSymbol)
        ) {
          return;
        }
      ...
      function handleNamespacedImportOrExport(
        namespacedImportOrExport: ts.ExportDeclaration | ts.ImportDeclaration,
        namespaceExports: Map<string, string>,
        symbol: ts.Symbol,
      ): void {
        ...
        if (
          ts.isImportDeclaration(namespacedImportOrExport) &&
          referencedSourceFileSymbol.exports.has(
      ...
      function processExportSymbol(
        ...
        symbol.declarations?.forEach((decl: ts.Declaration) => {
          if (
            ...
            decl.parent.moduleSpecifier !== undefined
          ) {
          ...
          if (ts.isExportSpecifier(decl)) {
            ...
            if (namespaceImport !== undefined) {
              // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
              handleNamespacedImportOrExport(
                namespaceImport.parent.parent as ts.ImportDeclaration,
                namespaceExports,
                symbol,
              );
            }
      ...
      // eslint-disable-next-line complexity
      function getIdentifierOfNamespaceImportFromInlinedModule(
        nsSymbol: ts.Symbol,
      ): ts.Identifier | null {
        ...
        for (const decl of getDeclarationsForSymbol(nsSymbol)) {
          ...
          if (
            ts.isNamespaceExport(decl) &&
            !isReferencedModuleImportable(decl.parent)
          ) {
          ...
          if (
            ts.isNamespaceImport(decl) &&
            !isReferencedModuleImportable(
              decl.parent.parent as ts.ImportDeclaration,
            )
          ) {
            return decl.name;
          }
          ...
          if (ts.isExportSpecifier(decl)) {
            ...
            if (decl.parent.parent.moduleSpecifier !== undefined) {
              ...
              if (
                getIdentifierOfNamespaceImportFromInlinedModule(
            ...
            const result = getDeclarationsForSymbol(
              getImportExportReferencedSymbol(decl, typeChecker),
            ).some((importDecl: ts.Declaration) => {
      ...
      const namespaceNameIdentifier =
        getIdentifierOfNamespaceImportFromInlinedModule(namespaceSymbol);
    ...
    function syncExports(): void {
      for (const exp of rootFileExports) {
        ...
        const namespaceLocalName =
          exp.symbol.flags & ts.SymbolFlags.ValueModule &&
          exp.symbol.exports !== undefined
      ...
      if (isNodeUsed(sourceFile)) {
        switch (currentModule.type) {
          ...
          case ModuleType.ShouldBeInlined: {
            ...
            if (
              sourceFileSymbol === null ||
              sourceFileSymbol.exports === undefined
            ) {
              throw new Error(
            ...
            forEachImportOfStatement(sourceFile, (imp: ImportOfStatement) => {
              ...
              if (ts.isNamespaceExport(imp) || ts.isNamespaceImport(imp)) {
                namespaceIdentifier = imp.name as ts.Identifier;
              }
            });
    ...
    if (entryConfig.failOnClass) {
      ...
      if (classes.length !== 0) {
        const classesNames = classes.map((c: ts.ClassDeclaration) =>
          c.name === undefined ? "anonymous class" : c.name.text,
        );
        throw new Error(
    ...
    function isExportedWithLocalName(
      namedDeclaration: ts.NamedDeclaration,
      exportedName: string,
    ): boolean {
    ...
    const output = generateOutput(
      {
        ...
        getStatementSettings: (statement: ts.Statement): StatementSettings => {
          ...
          const isExplicitlyExportedWithOriginalName =
            statementExports.find((exp: SourceFileExport) => {
              if (ts.isVariableStatement(statement)) {
                for (const variableDeclaration of statement.declarationList
                  .declarations) {
                  if (ts.isIdentifier(variableDeclaration.name)) {
                  ...
                  warnLog(
                    `Unhandled variable identifier type detected (${ts.SyntaxKind[variableDeclaration.name.kind]}). Please report this issue to https://github.com/timocov/dts-bundle-generator`,
                  );
                }
              ...
              return (
                ...
                isExportedWithLocalName(statement, exp.exportedName)
              );
            }) !== undefined;
          ...
          const onlyExplicitlyExportedShouldBeExported =
          ...
          return {
            shouldHaveExportKeyword:
              isExplicitlyExportedWithOriginalName ||
        ...
        needStripConstFromConstEnum: (constEnum: ts.EnumDeclaration) => {
          if (
            !program.getCompilerOptions().preserveConstEnums ||
            !outputOptions.respectPreserveConstEnum
          ) {
            return false;
          }
        ...
        needStripImportFromImportTypeNode: (node: ts.ImportTypeNode) => {
          ...
          return (
            getReferencedModuleInfo(node, criteria, typeChecker)?.type ===
            ModuleType.ShouldBeInlined
          );
        },
      },
    ...
    if (renamedAndNotExplicitlyExportedTypes.length !== 0) {
      warnLog(
        `The following type nodes were renamed because of the name collisions and will not be exported from the generated bundle:\n- ${
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          renamedAndNotExplicitlyExportedTypes
            .map(
              (node) =>
                `${getNodeName(node)!.getText()} (from ${node.getSourceFile().fileName})`,
            )
            .join("\n- ")
