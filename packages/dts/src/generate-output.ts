...
import { getPackageInfo } from "./helpers/package-info";
import {
  getModifiers,
  getNodeName,
  modifiersToMap,
  ...
  if (params.wrappedNamespaces.size !== 0) {
    resultOutputParts.push(
      Array.from(params.wrappedNamespaces.entries())
        .map(
          ([namespaceName, exportedNames]: [string, Map<string, string>]) => {
            return `declare namespace ${namespaceName} {\n\texport { ${Array.from(
              exportedNames.entries(),
            )
              .map(([exportedName, localName]: [string, string]) =>
                renamedExportValue(exportedName, localName),
              )
              .sort()
              .join(", ")} };\n}`;
          },
...
function getStatementText(
  ...
  const { shouldHaveExportKeyword, shouldHaveJSDoc } =
    helpers.getStatementSettings(statement);

  // re-export statements do not contribute to top-level names scope so we don't need to resolve their identifiers
  const needResolveIdentifiers =
    ...
    },
    {
      ...
      substituteNode: (hint: ts.EmitHint, node: ts.Node) => {
        ...
        if (needResolveIdentifiers) {
          if (ts.isPropertyAccessExpression(node)) {
            ...
            if (resolvedName !== null && resolvedName !== node.getText()) {
              ...
              let result: ts.PropertyAccessExpression | ts.Identifier =
        ...
        const resolvedStatementName =
          nodeName !== undefined
            ? helpers.resolveIdentifierName(nodeName as ts.Identifier) ||
              undefined
            : undefined;
  ...
  const statementText = printer
    .printNode(ts.EmitHint.Unspecified, statement, statement.getSourceFile())
    .trim();
  ...
  if (imports.namedImports.size !== 0) {
    result.push(
      `import { ${Array.from(imports.namedImports.entries())
        .map(([localName, importedName]: [string, string]) =>
          renamedImportValue(importedName, localName),
        )
        .sort()
        .join(", ")} } ${fromEnding}`,
    );
