...
const namedDeclarationKinds: ts.SyntaxKind[] = [
...
export function hasNodeModifier(
  ...
  return Boolean(
    modifiers &&
    modifiers.some(
      (nodeModifier: ts.Modifier) => nodeModifier.kind === modifier,
    ),
  );
}
...
export function getNodeName(node: ts.Node): NodeName | undefined {
  ...
  if (nodeName === undefined) {
    ...
    const defaultModifier = modifiers?.find(
      (mod: ts.Modifier) => mod.kind === ts.SyntaxKind.DefaultKeyword,
    );
...
export function getActualSymbol(
  symbol: ts.Symbol,
...
export function getDeclarationNameSymbol(
  name: NodeName,
  typeChecker: ts.TypeChecker,
): ts.Symbol | null {
...
export function splitTransientSymbol(
  symbol: ts.Symbol,
  typeChecker: ts.TypeChecker,
): Set<ts.Symbol> {
  ...
  for (const declaration of declarations) {
    if (
      !isNodeNamedDeclaration(declaration) ||
 ...
 */
export function isDeclareGlobalStatement(
  statement: ts.Statement,
): statement is ts.ModuleDeclaration {
  return (
    ts.isModuleDeclaration(statement) && isGlobalScopeAugmentation(statement)
  );
}
...
export function getExportsForSourceFile(
  typeChecker: ts.TypeChecker,
  ...
  const result = typeChecker
    ...
    .map<SourceFileExport>((symbol: ts.Symbol) => ({
      symbol,
  ...
  if (sourceFileSymbol.exports !== undefined) {
    ...
    if (defaultExportSymbol !== undefined) {
      const defaultExport = result.find(
        (exp: SourceFileExport) => exp.symbol === defaultExportSymbol,
      );
  ...
  result.forEach((exp: SourceFileExport) => {
    ...
    if (
      symbolsDeclarations.length > 1 &&
      importSpecifierDeclaration !== undefined
    ) {
      ...
      if (referencedModule !== null) {
        ...
        if (referencedModuleSymbol !== null) {
          const importedName = (
            importSpecifierDeclaration.propertyName ??
            importSpecifierDeclaration.name
          ).getText();
          const exportedItemSymbol = typeChecker
            .getExportsOfModule(referencedModuleSymbol)
            .find(
              (exportSymbol: ts.Symbol) =>
                exportSymbol.getName() === importedName,
            );
...
function resolveDeclarationByIdentifierSymbol(
...
export function getExportsForStatement(
  ...
  if (ts.isVariableStatement(statement)) {
    ...
    const allDeclarationsHaveSameExportType =
...
function getExportsForName(
  ...
  return exportedSymbols.filter(
    (rootExport: SourceFileExport) => rootExport.symbol === declarationSymbol,
  );
}
...
export function modifiersToMap(
...
function recreateRootLevelNodeWithModifiersImpl(
  node: ts.Node,
  modifiersMap: ModifiersMap,
  newName?: string,
): ts.Node {
...
export function getRootSourceFile(
  program: ts.Program,
  rootFileName: string,
): ts.SourceFile {
...
export function getNodeOwnSymbol(
  node: ts.Node,
  typeChecker: ts.TypeChecker,
): ts.Symbol {
...
export function getClosestModuleLikeNode(
  node: ts.Node,
): ts.SourceFile | ts.ModuleDeclaration {
...
export function getClosestSourceFileLikeNode(
  node: ts.Node,
): ts.SourceFile | ts.ModuleDeclaration {
  // we need to find a module block and return its module declaration
  // we don't need to handle empty modules/modules with jsdoc/etc
  while (
    !(ts.isModuleBlock(node) && ts.isStringLiteral(node.parent.name)) &&
...
export function getSymbolExportStarDeclarations(
  symbol: ts.Symbol,
): ts.ExportDeclaration[] {
...
export function getDeclarationsForExportedValues(
  ...
  const nodeForSymbol = ts.isExportAssignment(exp)
    ? exp.expression
    : exp.moduleSpecifier;
...
export function resolveGlobalName(
  ...
  return (typeChecker as Ts54CompatTypeChecker).resolveName(
    name,
    undefined,
    tsSymbolFlagsAll,
    false,
