 ...
 */
function applyAllowedCallees(mapping: Record<string, string[]>) {
...
function doesReturnJSX(
  ...
  return body.some((statement) => {
    let currentNode: babel.types.Node | null | undefined;
 ...
 */
function isJSX(t: BabelTypes, node: babel.types.Node | null | undefined) {
...
function isAllowedCallExpression(
  ...
  const callee: babel.types.Expression = path.node
...
function addDisplayNamesToFunctionComponent(
  ...
  path.find((parentPath) => {
    ...
    if (parentPath.isCallExpression()) {
      ...
      const callee: babel.types.Expression | babel.types.V8IntrinsicIdentifier =
...
function hasBeenAssignedPrev(
  ...
  return assignmentPath.getAllPrevSiblings().some((sibling) => {
    const expression = sibling.get("expression") as babel.NodePath;
    ...
    return (expression.get("left") as babel.NodePath).matchesPattern(pattern);
  });
}
